...
export function SystemHealth() {
  ...
  const checkHealth = async () => {
    try {
      ...
      if (error) {
        if (error.code === 'PGRST205') { // Missing Table
            setStatus('error')
        ...
        setDetails(`${t.systemHealth.connected}. ${count ?? 0} ${t.systemHealth.activeProducts}`)
      }
    } catch (e: any) {
        setStatus('error')
        ...
        <DialogContent>
            <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                    <Database className="w-5 h-5 text-emerald-600" /> {t.systemHealth.diagnostic}
                </DialogTitle>
...
`}
                ...
                <AnimatedButton onClick={checkHealth} className="w-full h-14 rounded-xl bg-emerald-600 text-white shadow-lg shadow-emerald-500/20 font-black text-lg">
