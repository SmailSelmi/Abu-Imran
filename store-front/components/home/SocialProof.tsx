...
export function DeliveryZones() {
  ...
  const [mainWilayas, setMainWilayas] = useState<string[]>(["الجزائر", "البليدة", "تيبازة", "بومرداس", "وهران"])
        ...
        <div className="container px-4 mx-auto flex flex-col lg:flex-row items-center gap-16">
            <div className={cn("flex-1 space-y-8", isRTL ? "text-right" : "text-left")}>
                ...
                <h2 className="text-4xl md:text-5xl font-black leading-tight">
                    {t.socialProof.deliveryTitle} <span className="text-blue-600">{highlightedWilayas.length > 0 ? highlightedWilayas.length : '58'} {t.socialProof.wilaya}</span>
                </h2>
                <p className="text-xl text-muted-foreground leading-relaxed">
                    {t.socialProof.deliveryDescription}
                </p>
                
                <div className={cn("flex flex-wrap gap-3 pt-4", isRTL ? "justify-start" : "justify-start")}>
                    {mainWilayas.map(w => (
                        <span key={w} className="px-4 py-2 bg-muted/50 border border-border/50 rounded-full text-sm font-medium flex items-center gap-2 hover:bg-white hover:shadow-sm transition-all cursor-default">
            ...
            <div className="flex-1 relative h-[500px] w-full bg-white dark:bg-zinc-900 rounded-xl shadow-md overflow-hidden border-8 border-white dark:border-zinc-800 rotate-1 hover:rotate-0 transition-all duration-500 flex items-center justify-center p-4">
                 ...
                 <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent pointer-events-none flex items-end justify-center pb-12">
                     <div className="bg-white/95 dark:bg-zinc-900/95 backdrop-blur-md px-8 py-6 rounded-xl shadow-md text-center transform hover:-translate-y-2 transition-transform border border-border">
                         <p className="font-bold text-2xl mb-1 text-foreground">{t.socialProof.fastDelivery}</p>
                         <p className="text-sm font-medium text-muted-foreground">{t.socialProof.deliveryTime}</p>
                     </div>
                 </div>
            </div>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {testimonials.map((test, i) => (
                        <div key={i} className={cn(
                            ...
                            <div className={cn("flex gap-1 text-yellow-400 mb-6", isRTL ? "justify-start" : "justify-start")}>
                                {[...Array(5)].map((_, i) => <Star key={i} className="w-4 h-4 fill-current" />)}
                            </div>
                            <p className="text-lg font-medium text-foreground/90 mb-8 leading-relaxed relative z-10">&quot;{test.text}&quot;</p>
                            <div className={cn("flex items-center gap-4 border-t border-border/50 pt-6", isRTL ? "flex-row" : "flex-row")}>
                                <div className={cn("w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold shrink-0", test.bg)}>
                                    {test.name.charAt(0)}
                                </div>
                                <div className={isRTL ? "text-right" : "text-left"}>
                                    <div className="font-bold text-lg">{test.name}</div>
                                    <div className="text-sm text-muted-foreground">{test.loc}</div>
                                </div>
                            </div>
                        </div>
                    ))}
