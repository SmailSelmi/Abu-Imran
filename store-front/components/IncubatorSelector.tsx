...
export function IncubatorSelector({ onSelect, selectedId, products }: IncubatorSelectorProps) {
      ...
      {products.map((incubator) => (
        ...
        >
           <div className="aspect-square relative bg-gradient-to-br from-gray-100 to-gray-50 flex items-center justify-center p-8">
                {incubator.image_url ? (
                    <Image src={incubator.image_url} alt={incubator.name_en} fill className="object-cover mix-blend-multiply" />
                ) : (
            ...
            <div className="p-6 text-center">
                <Badge variant="outline" className="mb-3 border-blue-200 text-blue-700 bg-blue-50">Automatic</Badge>
                ...
                <div className="pt-4 border-t border-dashed w-full">
                    <span className="font-mono text-xl font-bold text-blue-600 block">{incubator.price.toLocaleString()} <span className="text-xs text-muted-foreground">DA</span></span>
                </div>
            </div>
        </motion.div>
      ))}
