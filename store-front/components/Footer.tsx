...
export function Footer() {
  ...
  return (
    <footer className="w-full bg-zinc-950 text-white pt-24 pb-12 relative overflow-hidden" dir={isRTL ? 'rtl' : 'ltr'}>
      {/* Top Gradient Border */}
      ...
      <div className="container px-4 md:px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-16 mb-20">
          ...
          <div className="space-y-8">
            ...
            <div className="flex gap-4">
              {socialLinks.map(({ icon: IconComp, name, href }, i) => (
                <Link key={i} href={href} aria-label={`Follow us on ${name}`} className="flex h-12 w-12 items-center justify-center rounded-xl bg-white/5 border border-white/10 hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all hover:-translate-y-1 shadow-lg ring-1 ring-white/10">
          ...
          {/* Quick Links */}
          <div className="space-y-8">
            <h3 className="font-black text-xs uppercase tracking-[0.3em] text-emerald-500">{t.footer.shop}</h3>
            <ul className="space-y-5">
              {[
                  { label: t.footer.links.eggs, href: "/shop?category=eggs" },
                  { label: t.footer.links.chicks, href: "/shop?category=chicks" },
                  { label: t.footer.links.adult, href: "/shop?category=chickens" },
                  { label: t.footer.links.machine, href: "/shop?category=machine" },
              ].map((link, i) => (
                <li key={i}>
                    <Link className="text-zinc-400 hover:text-white transition-all flex items-center gap-4 group font-black text-sm tracking-tight" href={link.href}>
          ...
          {/* Support */}
          <div className="space-y-8">
            <h3 className="font-black text-xs uppercase tracking-[0.3em] text-emerald-500">{t.footer.support}</h3>
             <ul className="space-y-5">
              {[
          ...
          {/* Contact Info */}
          <div className="space-y-8">
            <h3 className="font-black text-xs uppercase tracking-[0.3em] text-emerald-500">{t.footer.contact}</h3>
            <ul className="space-y-6">
              <li className="flex items-start gap-5">
                ...
                <div className="flex flex-col gap-1">
                    <span className="text-[10px] uppercase font-black tracking-widest text-zinc-500">{t.common.location}</span>
                    <span className="font-black text-sm text-zinc-300">{t.footer.delivery58}</span>
                </div>
              </li>
              <li className="flex items-start gap-5">
                <div className="p-3.5 bg-emerald-600/10 rounded-xl text-emerald-500 ring-1 ring-emerald-500/20">
