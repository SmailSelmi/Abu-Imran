...
export default function OrdersPage() {
  ...
  useEffect(() => {
    ...
    audioRef.current = new Audio('https://www.soundjay.com/misc/sounds/cash-register-05.mp3') 
    fetchOrders()
  ...
  const recalculateReliability = async (customerId: string) => {
      ...
      const terminalOrders = orders.filter((o: any) => ['delivered', 'cancelled'].includes(o.status))
      const deliveredCount = terminalOrders.filter((o: any) => o.status === 'delivered').length
      const reliabilityScore = terminalOrders.length > 0 
        ? Math.round((deliveredCount / terminalOrders.length) * 100) 
        : 100

      // Financial Metrics (LTV/CLV)
      const deliveredOrders = orders.filter((o: any) => o.status === 'delivered')
      const totalSpent = deliveredOrders.reduce((acc: number, curr: any) => acc + (Number(curr.total_amount) || 0), 0)
            ...
            <table className="w-full caption-bottom text-sm">
                <thead className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground glass-card sticky top-0 z-10 border-b border-border/50">
                    <tr>
                        ...
                        <th className="h-12 px-4 text-right align-middle text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">المعرف</th>
                        <th className="h-12 px-4 text-right align-middle text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] min-w-[200px]">الزبون</th>
                        <th className="h-12 px-4 text-right align-middle text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] min-w-[250px]">تفاصيل الشحن</th>
                        <th className="h-12 px-4 text-right align-middle text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] w-[150px]">المنتج</th>
                        <th className="h-12 px-4 text-right align-middle text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">الإجمالي</th>
                        <th className="h-12 px-4 text-right align-middle text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">الحالة</th>
                        <th className="h-12 px-4 text-right align-middle text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">التاريخ</th>
                        <th className="h-12 px-4 align-middle text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] text-left">إجراءات</th>
                    </tr>
                </thead>
                <tbody className="[&_tr:last-child]:border-0">
                    {filteredOrders.map((order) => (
                        <tr key={order.id} className={`border-b border-border/30 transition-all duration-300 hover:bg-emerald-50/10 group ${selectedOrders.includes(order.id) ? 'bg-emerald-50/20' : ''}`}>
                            ...
                            <td className="p-4 align-middle font-mono text-xs text-muted-foreground">#{order.id.toString().slice(0,8)}</td>
                             <td className="p-4 align-middle">
                                <div className="space-y-1">
                                    <div className="font-bold flex items-center gap-2">
                                        {order.customers?.full_name || order.customer_name}
                                        {order.customers?.reliability_score && order.customers.reliability_score >= 90 && <ShieldCheck className="h-3 w-3 text-green-500" />}
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <a 
                                ...
                                <div className="space-y-2 max-w-[250px]">
                                    {order.order_items && order.order_items.length > 0 ? (
                                        ...
                                        <div className="font-medium flex flex-col gap-0.5">
                                            <span className="text-sm">{getProductNameAr(order)}</span>
                                            <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-tight">
                                                {order.category} / {order.product_variant || 'Standard'}
                                            </span>
                                        </div>
                                    )}
                                </div>
                            ...
                            <td className="p-4 align-middle font-bold text-black dark:text-white">
                                {(order.total_amount || 0).toLocaleString()} <span className="text-[10px] text-muted-foreground">DA</span>
                            </td>
                            <td className="p-4 align-middle">
