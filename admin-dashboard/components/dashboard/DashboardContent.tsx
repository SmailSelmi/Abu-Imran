...
export default function DashboardContent({
  ...
  const exportDailyCSV = async () => {
    ...
    const { data: orders, error } = await supabase
      .from("orders")
      .select(
        "id, customer_name, phone_number, product_name, product_variant, quantity, total_amount, status",
      )
            ...
            <div className="relative z-10 flex flex-col justify-between h-full">
              <div>
                ...
                <div className="text-5xl font-black tracking-tighter mt-2">
                  ...
                  <span className="text-xl opacity-70 italic font-sans">
                    د.ج
                  </span>
                </div>
              </div>
        ...
        <motion.div variants={item} className="md:col-span-3 md:row-span-1">
          <Card className="h-full border-none shadow-premium bg-primary/5 dark:bg-primary/10 border border-primary/10 rounded-3xl relative overflow-hidden p-4 md:p-6 group">
            <div className="relative z-10 flex flex-col justify-between h-full">
              ...
              <div>
                <div className="text-2xl font-black tracking-tight truncate uppercase text-zinc-900 dark:text-zinc-100">
                  {stats.topBreed}
                </div>
        ...
        <motion.div variants={item} className="md:col-span-3 md:row-span-1">
          <Card className="h-full border-none shadow-premium bg-primary/5 dark:bg-primary/10 border border-primary/10 rounded-3xl relative overflow-hidden p-4 md:p-6 group">
            <div className="relative z-10 flex flex-col justify-between h-full">
              ...
              <div>
                <div className="text-3xl font-black tracking-tighter tabular-nums text-zinc-900 dark:text-zinc-100">
                  {stats.avgOrderValue.toLocaleString()}{" "}
                  <span className="text-sm opacity-40">د.ج</span>
                </div>
                <p className="text-[10px] text-primary/40 mt-2 font-black uppercase tracking-widest">
                  لكل عملية ناجحة
                </p>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="mt-3 flex -space-x-2">
                  {[1, 2, 3, 4].map((i) => (
                    <div
                      key={i}
                      className="w-8 h-8 rounded-full border-2 border-white dark:border-zinc-900 bg-muted flex items-center justify-center text-[10px] font-black overflow-hidden bg-gradient-to-br from-primary/10 to-primary/30"
                    />
                  ))}
                  <div className="w-8 h-8 rounded-full border-2 border-white dark:border-zinc-900 bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-[8px] font-black text-primary">
                    +{stats.activeUniqueCustomers}
                  </div>
                </div>
              </div>
            </div>
          ...
          <Card className="h-full border-none shadow-premium bg-red-50/50 dark:bg-red-950/10 border-2 border-red-100/20 rounded-3xl relative overflow-hidden p-4 md:p-6 group">
            <div className="relative z-10 flex flex-col justify-between h-full">
              ...
              <div>
                ...
                >
                  <span className="text-[10px] font-black text-red-900/40 uppercase tracking-widest group-hover/link:text-red-900 transition-colors">
                    إدارة المخزون
                  </span>
            ...
            <CardHeader className="p-10 pb-4 flex flex-row items-center justify-between">
              <div>
                <CardTitle className="text-3xl font-black tracking-tighter flex items-center gap-3">
                  <Activity className="w-8 h-8 text-emerald-500" /> النشاط
                  المباشر
                </CardTitle>
                <CardDescription className="font-bold text-lg opacity-40 mt-1">
                  تتبع التدفق الفوري للإنتاج
                </CardDescription>
              </div>
            </CardHeader>
                ...
                {recentActivity.map((order, i) => (
                  ...
                  >
                    <div className="text-center min-w-[70px]">
                      <p className="text-[10px] font-black uppercase text-muted-foreground opacity-40 tracking-widest">
                        {order.created_at
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3">
                        <p className="text-lg font-black truncate text-zinc-900 dark:text-zinc-100 uppercase group-hover:text-emerald-600 transition-colors tracking-tight">
                          {order.product_name}
                        </p>
                        <div
                          className={clsx(
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        <span className="text-[10px] font-bold text-muted-foreground opacity-60 uppercase tracking-widest">
                          بواسطة {order.customer_name || "Anonymous"}
                        </span>
                      </div>
                    </div>
                    <div className="text-left font-sans">
                      ...
                      <p className="text-[9px] font-black opacity-30 text-right uppercase">
                        د.ج
                      </p>
                    </div>
                  </motion.div>
            ...
            <Card className="border-none shadow-premium rounded-[2.5rem] bg-indigo-50/50 dark:bg-indigo-950/10 border-2 border-indigo-100/20 p-8 overflow-hidden">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-indigo-900 dark:text-indigo-100 tracking-tight">
                      التنبؤات
                    </h3>
                    <p className="text-[10px] text-indigo-600/50 font-black uppercase tracking-widest">
                      توقعات الطلب القادم
                    </p>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                {forecasts.map((f, i) => (
                  <div
                    key={i}
                    className="flex justify-between items-center bg-white/80 dark:bg-zinc-900/40 p-5 rounded-[2rem] shadow-sm border border-indigo-100/30 group"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-black text-indigo-900 dark:text-indigo-100 group-hover:text-indigo-600 transition-colors uppercase">
                          {f.name}
                        </p>
                        <p className="text-[10px] text-indigo-700/40 font-bold uppercase tracking-widest">
                          المخزون: {f.stock}
                        </p>
                      </div>
                    </div>
