...
export function CategorySwitcher({ selected, onSelect }: CategorySwitcherProps) {
  const categories = [
    { id: 'eggs', label: 'Hatching Eggs', icon: Egg, color: 'yellow' },
    { id: 'chicks', label: 'Chicks', icon: Bird, color: 'orange' },
    { id: 'chickens', label: 'Adults', icon: ChefHat, color: 'red' },
    { id: 'incubators', label: 'Incubators', icon: Thermometer, color: 'blue' },
  ] as const;
      ...
      {categories.map((cat) => (
        ...
        >
          <div className={clsx(
            ...
            selected === cat.id ? `bg-${cat.color}-100 dark:bg-${cat.color}-900/30 text-${cat.color}-600` : "bg-gray-100 dark:bg-zinc-800"
          )}>
