...
import { ArrowLeft, Phone, MapPin, Save, ShieldCheck, ShieldAlert, Package, Clock, MessageCircle, Tag, Plus, X } from "lucide-react"
      ...
      <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
               <Badge className={clsx(
                   ...
                   reliability >= 90 ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"
               )}>
          ...
          <div className="lg:col-span-1 space-y-8">
              <Card className="border-none shadow-2xl bg-white dark:bg-card rounded-xl overflow-hidden group">
                       ...
                       <div className="absolute -bottom-12 left-1/2 -translate-x-1/2">
                            <div className="w-24 h-24 bg-white dark:bg-card p-1.5 rounded-xl shadow-xl group-hover:scale-105 transition-transform">
                                 <div className="w-full h-full bg-muted rounded-xl flex items-center justify-center overflow-hidden">
                                      <Icon src="https://cdn.lordicon.com/bgebyztw.json" trigger="loop" size={64} />
                                 </div>
                            </div>
                       </div>
                  </div>
                  <CardContent className="pt-16 text-center space-y-6 pb-10">
                      <div>
                          <h2 className="text-3xl font-black tracking-tight">{customer?.full_name}</h2>
                          <p className="text-muted-foreground font-medium text-sm">ID: #{customer?.id.toString().slice(0,8)}</p>
                      </div>

                      <div className="grid grid-cols-2 gap-4">
                           <div className="bg-muted/40 p-4 rounded-xl">
                                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Lifetime Value</p>
                                <p className="text-xl font-black text-green-600">{customer?.total_spent?.toLocaleString() || 0} <span className="text-[10px]">DA</span></p>
                           </div>
                           <div className="bg-muted/40 p-4 rounded-xl">
                                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Frequency</p>
                                <p className="text-xl font-black">{customer?.total_orders || 0} <span className="text-[10px]">Orders</span></p>
                           </div>
                      </div>
                      ...
                      <div className="space-y-4 text-left px-4">
                          <div className="flex items-center gap-4 group/item">
                              ...
                              <div>
                                  <p className="text-[10px] font-bold text-muted-foreground leading-none mb-1">DIRECT LINE</p>
                                  <span className="font-black text-sm">{customer?.phone || 'UNAVAILABLE'}</span>
                              </div>
                          </div>
                          <div className="flex items-center gap-4 group/item">
                              ...
                              <div>
                                  <p className="text-[10px] font-bold text-muted-foreground leading-none mb-1">PRIMARY CLUSTER</p>
                                  <span className="font-black text-sm uppercase">{customer?.wilaya} {customer?.commune ? `/ ${customer.commune}` : ''}</span>
                              </div>
                          </div>

                          <div className="pt-4 border-t border-muted/50">
                               <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-3">Customer Tags</p>
                               <div className="flex flex-wrap gap-2">
                                   {tags.map(tag => (
                                       <Badge key={tag} className="bg-muted hover:bg-muted/80 text-muted-foreground border-none rounded-lg px-2 py-1 flex items-center gap-1 group/tag">
                                           {tag}
                                           <X className="w-3 h-3 cursor-pointer opacity-0 group-hover/tag:opacity-100 transition-opacity" onClick={() => setTags(tags.filter(t => t !== tag))} />
                                       </Badge>
                                   ))}
                      ...
                      <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-4">
                              ...
                              <div className={clsx(
                                  ...
                                  reliability >= 90 ? "bg-green-500 text-white" : "bg-orange-500 text-white"
                              )}>
                                 {reliability >= 90 ? <ShieldCheck className="w-6 h-6" /> : <ShieldAlert className="w-6 h-6" />}
                              </div>
                          </div>
                      </div>
                      ...
                      <div className="flex justify-between items-center">
                           <CardTitle className="text-2xl font-black flex items-center gap-3">
                               <div className="w-10 h-10 rounded-xl bg-muted flex items-center justify-center">
                                    <Icon src="https://cdn.lordicon.com/wxnxiano.json" trigger="loop" size={28} />
                               </div>
                               Transaction History
                           </CardTitle>
                           <Badge variant="secondary" className="font-bold rounded-lg px-3">{orders.length} TOTAL</Badge>
                      </div>
                  </CardHeader>
                  <CardContent className="p-0">
                      <div className="divide-y divide-muted/50">
                          {orders.map((order) => (
                              <div key={order.id} className="p-8 hover:bg-muted/10 transition-all duration-300 flex flex-col md:flex-row md:items-center justify-between gap-6 group">
                                  <div className="flex items-center gap-6">
                                      <div className={clsx(
                                          ...
                                          order.category === 'machine' ? 'bg-purple-50 text-purple-600' : 'bg-orange-50 text-orange-600'
                                      )}>
                                          <Icon 
                                              src={order.category === 'eggs' ? "https://cdn.lordicon.com/hursldjj.json" : 
                                                   order.category === 'chicks' ? "https://cdn.lordicon.com/rxufjlal.json" : "https://cdn.lordicon.com/msoeawqm.json"} 
                                              trigger="hover" 
                                              size={32} 
                                          />
                                      </div>
                                      <div className="min-w-0">
                                          <p className="text-xl font-black truncate">{order.product_name}</p>
                                          <div className="flex items-center gap-2 mt-1">
                                               <Badge variant="outline" className="text-[10px] font-black border-muted-foreground/20 px-1.5 py-0 leading-none h-4 uppercase">{order.category}</Badge>
                                               <span className="text-xs text-muted-foreground font-medium flex items-center gap-1">
                                                   <Clock className="w-3 h-3" /> {order.created_at ? formatDistanceToNow(new Date(order.created_at), { addSuffix: true }) : 'N/A'}
                                               </span>
                                          </div>
                                      </div>
                                  </div>
                                  ...
                                  <div className="flex items-center justify-between md:text-right md:flex-col md:items-end gap-2 w-full md:w-auto border-t md:border-none pt-4 md:pt-0">
                                      <div>
                                           <p className="text-2xl font-black tracking-tighter text-green-600">{(order.total_amount || 0).toLocaleString()} <span className="text-xs font-bold opacity-60">DA</span></p>
                                           <p className="text-[10px] font-bold text-muted-foreground leading-none">UNIT TYPE: x{order.quantity || 1}</p>
                                      </div>
                          ...
                          {orders.length === 0 && (
                              <div className="py-20 text-center space-y-4 opacity-50">
                                  <Icon src="https://cdn.lordicon.com/msoeawqm.json" trigger="loop" size={80} />
                                  <p className="text-xl font-black">STAGNANT TRANSACTION LOG</p>
                              </div>
                          )}
                      </div>
                  </CardContent>
