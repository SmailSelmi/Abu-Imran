...
export default function IncubatorsPage() {
  ...
  const handleDelete = async () => {
      ...
      else { toast.success('Asset moved to recycle bin'); fetchProducts(); }
  ...
  const resetForm = () => {
      ...
      setEditingId(null)
      setFormData({
        name_en: '', slug: '', category: 'machine', subcategory: 'Incubator', price: 0, stock: 0, image_url: '/images/incubator.svg'
      })
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <h1 className="text-4xl font-black tracking-tighter flex items-center gap-4">
                 وحدات الحاضنات <span className="text-emerald-200 text-2xl font-black">/</span> {products.length}
            </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              <Card key={product.id} className="relative overflow-hidden group border border-border/40 shadow-sm rounded-xl bg-white dark:bg-card hover:shadow-sm transition-all duration-500">
                    <CardHeader className="p-7 pb-2 relative z-10">
                      ...
                      <CardTitle className="text-2xl font-black leading-tight mt-4 tracking-tight group-hover:text-primary transition-colors flex flex-col gap-1">
                        {product.name_en}
                        {product.slug && (
                            <span className="text-[10px] font-mono opacity-30">/{product.slug}</span>
                        )}
                   </CardTitle>
                  </CardHeader>
                  <CardContent className="p-7 pt-2 relative z-10">
                      <div className="flex justify-between items-end mt-6">
                          <div>
                               <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest mb-1 opacity-50">قيمة الوحدة</p>
          ...
          {products.length === 0 && !loading && (
              <div className="col-span-full text-center py-24 opacity-20 space-y-4">
                  <Icon src="https://cdn.lordicon.com/dxjqvkzv.json" trigger="loop" size={80} />
                  <h3 className="text-2xl font-black font-mono">NO ASSETS DETECTED</h3>
              </div>
          )}
      </div>
    </div>
