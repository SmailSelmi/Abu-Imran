...
import { 
  Trash2, Phone, MapPin, Calendar, Egg,
...
export default function HatchingPage() {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    const { error } = await supabase.from('hatching_bookings').update({ status: newStatus }).eq('id', id)
    if (error) {
  ...
  const deleteBooking = async (id: string) => {
      setConfirmConfig({
          ...
          variant: "destructive",
          onConfirm: async () => {
              const { error } = await supabase.from('hatching_bookings').delete().eq('id', id)
            ...
            <table className="w-full text-sm min-w-[800px]">
                ...
                <tbody>
                    {filteredBookings.map((booking) => (
                        <tr key={booking.id} className="border-b border-border/30 hover:bg-emerald-50/10">
                            <td className="p-4">
                                <div className="font-bold">{booking.customers?.full_name}</div>
                            ...
                            <td className="p-4 text-xs">
                                <div className="flex items-center gap-1"><Calendar className="w-3 h-3" /> {format(new Date(booking.start_date), 'dd/MM/yyyy')}</div>
                                <div className="flex items-center gap-1 opacity-60"><Calendar className="w-3 h-3" /> {format(new Date(booking.end_date), 'dd/MM/yyyy')}</div>
                            </td>
