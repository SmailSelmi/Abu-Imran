        ...
        for (const family of fams) {
          ...
          if (familyProducts && familyProducts.length > 0) {
            ...
            familyProducts.forEach((p) => {
              ...
              if (p.category === "chickens")
                breedItem.price_adult = p.price || 0;
            });
      ...
      <div className="container px-4 mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8">
          <div className="max-w-xl space-y-3">
            <Badge
              variant="outline"
              className="px-3 py-0.5 border-emerald-500/20 bg-emerald-500/5 text-emerald-600 text-[10px] uppercase font-black tracking-widest"
            >
              تشكيلة عالية الجودة
            </Badge>
            <h2 className="text-3xl md:text-5xl font-black tracking-tighter leading-tight">
              سلالات تراثية <br />
              <span className="text-emerald-600 italic">نادرة</span>
            </h2>
            <p className="text-sm md:text-base text-muted-foreground font-medium max-w-sm leading-relaxed">
              اختبر التميز مع مجموعتنا المختارة بعناية من الدواجن الفاخرة والمعدات المتخصصة.
            </p>
          </div>
          <Link href="/shop" className="group w-full md:w-auto">
            <Button
              variant="outline"
              className="w-full h-12 md:h-14 px-8 rounded-xl border-border bg-white dark:bg-black hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all duration-300 font-bold"
            >
              عرض جميع المنتجات
