...
export default function AlgeriaMap({ onWilayaSelect, data = {}, dataLabel = 'Orders', className = '' }: AlgeriaMapProps) {
      ...
      {hoveredWilaya && (
        ...
        >
          <div className="flex items-center justify-between gap-4">
             <span className="font-black text-emerald-400 text-lg">{hoveredWilaya.nameAr}</span>
             <span className="text-[10px] font-bold opacity-40 uppercase tracking-widest text-right">#{hoveredWilaya.id}</span>
          </div>
          <span className="text-xs font-bold opacity-80 uppercase tracking-tight">{hoveredWilaya.nameLatin}</span>
          <div className="mt-2 pt-2 border-t border-white/10 flex items-center justify-between">
              <span className="text-[10px] uppercase font-black opacity-50 tracking-widest">{dataLabel}</span>
              <span className="text-xl font-black text-emerald-400">{hoveredWilaya.value?.toLocaleString() || 0}</span>
          </div>
        </div>
      )}
    </div>
